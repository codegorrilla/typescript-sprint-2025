...
const dogProfile = (profile: pedigree): void => {
  ...
  console.log(
    `${profile.name} is a ${profile.breed} and it's color is ${profile.color}`
  );
};
...
const calcProfit = (earning: game): any => {
  return (
    `The ${earning.name} which is a ${earning.type} game, has earned a net profit of ` +
    earning.profit * 0.3232 +
    `USD for the year ${earning.year}`
  );
};
